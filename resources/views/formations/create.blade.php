<x-app-layout>
    <div class="max-w-3xl mx-auto p-8 bg-white rounded-xl shadow-lg mt-12 border border-gray-200">
        ...
        <h1 class="text-3xl font-semibold text-gray-900 mb-8">Créer une nouvelle formation</h1>

        <form action="{{ route('formations.store') }}" method="POST" enctype="multipart/form-data" class="space-y-7">
            @csrf
            ...
            <div>
                ...
                <textarea 
                    ...
                    placeholder="Description de la formation">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select
                    ...
                    class="w-full rounded-md border border-gray-300 bg-gray-50 text-gray-900
                           focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400
                           transition duration-150 ease-in-out
                           @error('category_formation_id') border-red-500 focus:ring-red-400 focus:border-red-400 @enderror"
                >
