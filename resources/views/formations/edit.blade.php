<x-app-layout>
    <div class="max-w-3xl mx-auto p-6 mt-12 bg-gradient-to-br from-white/90 to-gray-50 rounded-xl shadow-2xl border border-gray-200/50 backdrop-blur-md">
        ...
        <h1 class="text-4xl font-extrabold text-gray-900 mb-8 text-center">Modifier la Formation</h1>
        ...
        <form action="{{ route('formations.update', $formation->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="form-control">
                ...
                <input 
                    type="text" name="name" id="name" value="{{ old('name', $formation->name) }}" required
            ...
            <div class="form-control">
                ...
                <input 
                    type="number" name="price" id="price" step="0.01" min="0" value="{{ old('price', $formation->price) }}" required
                    class="w-full px-4 py-2 rounded-lg border border-gray-300 bg-white/80 text-gray-900
                           placeholder-gray-400 focus:ring-2 focus:ring-indigo-500 focus:border-transparent
                           transition-all duration-200 ease-in-out shadow-sm
