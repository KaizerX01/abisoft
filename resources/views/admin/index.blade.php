<x-admin-layout>
        ...
        <main class="flex-1 p-8 space-y-10">
            ...
            <section class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    <a href="{{ route('products.create') }}" class="btn btn-outline btn-primary flex flex-col items-center gap-2 p-4 h-auto">
                        <i class="fa-solid fa-plus text-2xl"></i>
                        <span class="text-sm">Nouveau Produit</span>
                    </a>
                    <a href="{{ route('services.create') }}" class="btn btn-outline btn-info flex flex-col items-center gap-2 p-4 h-auto">
                        <i class="fa-solid fa-plus text-2xl"></i>
                        <span class="text-sm">Nouveau Service</span>
                    </a>
                    <a href="{{ route('formations.create') }}" class="btn btn-outline btn-secondary flex flex-col items-center gap-2 p-4 h-auto">
                        <i class="fa-solid fa-plus text-2xl"></i>
                    ...
                    <tbody>
                        @foreach($products->take(5) as $product)
                            <tr class="hover">
                                ...
                                <td class="text-right"> {{-- Align actions to the right --}}
                                    <div class="flex items-center justify-end gap-2"> {{-- Use flexbox to align buttons and add gap --}}
                                        ...
                                        <form action="{{ route('products.destroy', $product->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce produit ?')" class="inline-block"> {{-- Use inline-block for proper spacing --}}
                                            @csrf
                                            @method('DELETE')
    ...
    <div class="p-8">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
                <h3 class="text-2xl font-bold text-gray-800 flex items-center gap-3">
                    <div class="w-8 h-8 bg-gradient-to-r from-pink-500 to-purple-500 rounded-xl flex items-center justify-center">
            ...
            <div class="flex gap-3">
                <a href="{{ route('services.create') }}" class="btn btn-info btn-sm gap-2 hover:scale-105 transition-transform duration-200">
        ...
        @else
            <div class="text-center py-16">
                ...
                <a href="{{ route('services.create') }}" class="btn btn-info btn-lg gap-2 hover:scale-105 transition-transform duration-200">
    ...
    <div class="p-8">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            ...
            <div class="flex gap-3">
                <a href="{{ route('formations.create') }}" class="btn btn-secondary btn-sm gap-2 hover:scale-105 transition-transform duration-200">
        ...
        @else
            <div class="text-center py-16">
                ...
                <a href="{{ route('formations.create') }}" class="btn btn-secondary btn-lg gap-2 hover:scale-105 transition-transform duration-200">
...
<section class="card bg-white shadow-xl border border-gray-100 rounded-3xl overflow-hidden mb-8">
    <div class="card-body">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <h3 class="text-2xl font-bold text-gray-800 flex items-center gap-3">
                <i class="fa-solid fa-newspaper text-accent text-xl"></i>
                Articles de Blog Récents
            </h3>
            <div class="flex gap-2">
                <a href="{{ route('blog.create') }}" class="btn btn-accent btn-sm">
                    <i class="fa-solid fa-plus"></i> Nouvel Article
                </a>
                <a href="{{ route('blog.index') }}" class="btn btn-outline btn-sm">Voir tous</a>
            </div>
        </div>
        ...
        @if(count($blogs))
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                @foreach($blogs->take(5) as $blog)
                    <div class="border border-gray-200 rounded-2xl bg-gray-50 p-5 flex flex-col justify-between">
                        <div class="mb-4">
                            <h4 class="text-lg font-semibold text-gray-800 truncate">
                                {{ $blog->title }}
                            </h4>
                            <p class="text-sm text-gray-500 mt-1">
                                {{ $blog->category?->name ?? 'Sans catégorie' }} •
                                {{ $blog->created_at->format('d M Y') }}
                            </p>
                        </div>

                        <div class="flex items-center justify-between">
                            <span class="badge badge-primary text-white text-xs px-3 py-1">Publié</span>
                            <div class="flex gap-2">
                                <a href="{{ route('blog.edit', $blog->id) }}"
                                   class="btn btn-sm btn-outline text-sm">
                                    <i class="fa-solid fa-edit mr-1"></i> Modifier
                                </a>
                                <form action="{{ route('blog.destroy', $blog->id) }}" method="POST" onsubmit="return confirm('Supprimer cet article ?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-outline text-red-600 border-red-300 hover:bg-red-100">
        ...
        @else
            {{-- Empty State --}}
            <div class="text-center py-16">
                <i class="fa-solid fa-newspaper text-6xl text-gray-300 mb-6"></i>
                <p class="text-gray-500 text-lg">Aucun article trouvé.</p>
                <a href="{{ route('blog.create') }}" class="btn btn-accent mt-6">Créer le premier article</a>
            </div>
        @endif
    </div>
</section>
