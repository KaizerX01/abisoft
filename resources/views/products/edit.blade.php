<x-app-layout>
    <div class="max-w-3xl mx-auto p-6 mt-12 bg-gradient-to-br from-white/90 to-gray-50 rounded-xl shadow-2xl border border-gray-200/50 backdrop-blur-md">
        ...
        <h1 class="text-4xl font-extrabold text-gray-900 mb-8 text-center">Modifier le Produit</h1>
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="form-control">
                ...
                <input 
                    type="number" name="price" id="price" step="0.01" min="0" value="{{ old('price', $product->price) }}" required
                    class="w-full px-4 py-2 rounded-lg border border-gray-300 bg-white/80 text-gray-900
                           placeholder-gray-400 focus:ring-2 focus:ring-indigo-500 focus:border-transparent
                           transition-all duration-200 ease-in-out shadow-sm
            ...
            <div class="form-control">
                <label for="category_product_id" class="block text-lg font-medium text-gray-700 mb-2">Catégorie</label>
                <select
                    name="category_product_id" id="category_product_id" required
                    class="w-full px-4 py-2 rounded-lg border border-gray-300 bg-white/80 text-gray-900
                           focus:ring-2 focus:ring-indigo-500 focus:border-transparent
                           transition-all duration-200 ease-in-out shadow-sm
                           @error('category_product_id') border-red-500 focus:ring-red-500 @enderror"
                >
