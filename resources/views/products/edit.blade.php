<x-app-layout>
    <div class="max-w-3xl mx-auto p-8 bg-white rounded-xl shadow-lg mt-12 border border-gray-200">
        <h1 class="text-3xl font-semibold text-gray-900 mb-8">Modifier le produit</h1>

        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-7">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="category_product_id" id="category_product_id" required
                        class="w-full rounded-md border border-gray-300 bg-gray-50 text-gray-900 focus:ring-indigo-400 focus:border-indigo-400 transition duration-150 ease-in-out @error('category_product_id') border-red-500 focus:ring-red-400 focus:border-red-400 @enderror">
                ...
                @error('category_product_id') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </div>
